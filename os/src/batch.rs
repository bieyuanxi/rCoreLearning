...
use crate::{error, println, sbi::shutdown};
...
struct AppManager {
    num_apps: usize,
    current_app: usize,
    app_start_address: [usize; MAX_APP_NUM + 1],
}

impl AppManager {
    fn print_app_info(&self) {
        println!("[kernel] num_apps = {}", self.num_apps);
        for i in 0..self.num_apps {
            println!(
                "[kernel] app_{} [{:#x}, {:#x})",
                i,
                self.app_start_address[i],
                self.app_start_address[i + 1]
            );
        }
    }
    ...
    fn load_app(&self, app_id: usize) {
        ...
        unsafe {
            ...
            let app_src = core::slice::from_raw_parts(
                self.app_start_address[app_id] as *const u8,
                self.app_start_address[app_id + 1] - self.app_start_address[app_id],
            );
            let app_dest =
